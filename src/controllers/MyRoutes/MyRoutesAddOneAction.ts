...
export async function myRoutesAddOneAction(
  request: Request,
  response: Response
) {
  try {
    ...
    await createQueryBuilder().relation(Rider, "routes").of(rider).add(route);

    return response.status(200).json({ added: route });
  } catch (error) {
    console.log(error);
